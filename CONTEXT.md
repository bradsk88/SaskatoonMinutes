...
- **Presentation** — one guest speaker's turn at an agenda item: a delegate's name, organization, stance (`support` / `concern` / `""` for informational), and a one-sentence summary. Lives on `AgendaItem.presentations` (`app/models.py`), extracted deterministically in `app/presentations.py` from the same PostMinutes prose the Delegation chip reads, plus a fallback pass over Request-to-Speak ("RTS") attachment filenames for delegates the prose only mentions in passing. Distinct from the **Delegation chip**: the chip is a single first-match soundbite folded into an item's Chips grid; a Presentation is the full, deduplicated roster, rendered as its own section on the detail page and surfaced as a count on the index card.
