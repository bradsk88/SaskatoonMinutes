...
- **Settled** — a Meeting is publishable once it has a **real** (post-meeting) cached summary, or seven days have passed. A provisional summary does not settle it: it was written before the meeting from the agenda alone, and an entry settled on it would say what the agenda promised, not what happened (ADR `0026`). A meeting settles on its own, without waiting on a sibling that sat the same day. Nothing is published thin and filled in later: a reader who saw the thin version would never be shown the full one. The seven days are what stops a meeting with no video from waiting forever; a meeting that settles that way, with no video, carries a not-recorded note in the feed (ADR `0026`).
- **Recording state** — what a Meeting's detail page says where the video would be, when there is no video. Two distinct states, told apart by how long the meeting has sat without one (ADR `0026`): **pending** (the City has not posted the recording yet; the site will transcribe and summarize it when it lands) and **not recorded** (the grace period, seven days, has passed with no video, so the meeting was never recorded). The same seven days the feed settles on, so the site and the feed agree on when a missing recording becomes a missing recording. A cancelled meeting and one that has not happened yet say nothing.
