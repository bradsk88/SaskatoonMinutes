...
- **Scheduled Meeting** — a Meeting that has not happened yet: announced on the upstream calendar, `meeting_id` already assigned, agenda possibly published, but no video, minutes, or votes. A distinct concept from Meeting, not a state of one — it carries no Outcome vocabulary, and nothing about it is an Outcome. Shown only on the **Future tab** (always the left-most tab), scoped to the union of bodies in `MEETING_TABS`, sorted by date, each row naming its body. A Scheduled Meeting with no published agenda yet appears as a bare row (date and body, no items). Scheduled Meetings never appear in the two settled Atom feeds (`feed.xml`, `feed-items.xml`) — they have not sat, so nothing about them is Settled. They do appear in the dedicated **Future Feed** (`feed-future.xml`, ADR `0024`): one entry per Scheduled Meeting, published when its agenda first appears (or ~3 days before the Request-to-Speak Deadline if none has), containing the full agenda in agenda order with provisional Descriptions, the deadline up top, soonest-first, with no entry cap beyond the lookahead window. Entries update in place on agenda revision (stable GUID) and are dropped once the meeting happens. The Future tab is exempt from the Index Card's Space Budget: its purpose is seeing everything clearly, not finding things at a glance, so a Scheduled Meeting shows all its agenda items in agenda order, title plus provisional Description, with no outcome badges and no ranking. Each Scheduled Meeting carries a static "how to register to speak" blurb linking to the City's submission form, plus its **Request-to-Speak Deadline**: 5:00 p.m. on the Monday of the meeting week, computed from the meeting date (unchanged on holidays, per the City). Once that date has passed, the blurb's link is replaced by a note that the deadline has passed — the deadline day itself still counts as open. Once a Scheduled Meeting appears in PastMeetings it is an ordinary Meeting and the concept no longer applies to it.

  Its agenda-item attachments may carry an **AttachmentGist**: a 5-Ws
  summary (What/Who/When/Where/Why, one terse line each, "—" where a W
  does not apply) of one attachment PDF, written so a citizen skimming
  the Future tab can tell whether the document concerns them without
  downloading it.  Generated once per eSCRIBE `DocumentId` at
  provisional-summary time (fetch + pypdf text extraction, 15-page cap,
  Gemini), cached per `meeting_id` in `attachment-gists/` on the
  `summaries` branch, and never revised — the flip to Meeting discards
  them with the provisional summaries.  Failure is silent by design: an
  unreadable PDF yields no gist and the attachment renders as a bare
  link, because a wrong gist is worse than none.  Always labeled
...
- **Speaker** — one member of the public who addressed council on an agenda item: name, organization, stance (`support` / `concern` / `""` for informational), and `said`, the bullets of what they argued. Built in two halves. The **roster** — who came — is deterministic and rebuilt on every page build (`app/speakers.py`): PostMinutes prose, plus a pass over Request-to-Speak ("RTS") attachment filenames for anyone the prose only mentions in passing. What they **said** costs a Gemini call, so it is cached on `ItemSummary.speakers` and merged back onto the roster by `merge_remarks`. A meeting no summarize run has reached has a roster and no substance; that is expected, not missing data.
  ...
  Not "presentation": the city never uses the word (the June 24 agenda has 0 uses against 27 for "RTS"), it describes what *staff* do with a report and a PowerPoint — which is the false positive the name test exists to reject — and it collided with the Delegation chip below. Distinct from the **Delegation chip**: the chip is a single first-match soundbite folded into an item's Chips grid; the roster is every speaker, deduplicated, each with their own card on the detail page and, for up to three of them per item, a row beneath that item on the index card.

  On the index card a Speaker is always a **row, never a chip** (ADR `0022`): a chip is a label on a thing, and a speaker is a thing. The row takes the title-only row's shape — name, organization, stance — coloured by the organization's own palette.
- **Jointly-Heard Items** — two agenda items a committee took as one discussion (G&P 2026-08-12: 6.3.2 with 7.1), detected automatically from overlapping bookmark windows plus a shared roster name — never from a manual list (ADR `0025`). The earliest-window item is the **primary** and carries the merged speaker list; partners carry a `heard_with` annotation and point at it. One speech answered both items, so each Speaker appears once, with their single stamp, on the primary card.
...
- **Feed Entry** — one published unit of the Atom feeds. Two feeds, differing only in granularity, never in what qualifies:
  - **Meeting Entry** (`/feed.xml`) — one entry per meeting the city sat, with that meeting's qualifying items as its body, newest first. A busy Tuesday on which several bodies sat is several entries, one per body, rather than one entry for the day. This is the default feed.
...
- **Index Card** — a Meeting's summary block on the index page, meant to get a reader into the right Meeting, not to list it. What it may show is bounded by a **Space Budget** of 15 units (roughly one mobile screen), spent on rows of different costs:
  ...
  - **Speaker Row** (1 unit) — one Speaker's name, organization, and stance, drawn beneath the item they answered and coloured by the organization's palette. Up to three per item; past the cap a free **"+N more" Row** points at the detail page, and the capped-away organizations still appear in the Speaker Digest.
  - **Consent Roll-up** (1 unit) — a single row standing in for the whole consent block ("N items approved in consent"), with a help icon explaining the jargon. Consent Items never get rows of their own: approved without debate, there is nothing to summarize and nothing to name. Like the Speaker Digest, never cut.
  - **Speaker Digest** (1 unit per row) — the Speakers with no inline row on the card: one slim row per represented organization not already shown, plus a residents roll-up. The union of inline Speaker Rows and the digest is always the full roster — hiding which orgs had a voice is not an acceptable saving, so the digest is never cut, only shrunk as inline coverage grows. Only voices actually heard count: a registered-only name (an RTS filing with no remarks and no chair introduction in the transcript) proves intent, not attendance, and is excluded.

  When a card exceeds its budget it spends down in a fixed order (ADR `0022`): the *least-attended* Detailed Rows demote to Title-Only first (their speakers return to the digest), then Speaker Rows are trimmed off the *most-attended* item one at a time — only where a trim actually saves space, since a speaker whose organization rejoins the never-cut digest saves nothing — then Title-Only Rows drop from the bottom, and demoting the most-attended item itself is the last resort. Top-up restores trimmed Speaker Rows before naming another item: engagement on an item the card already shows outranks breadth.
- **Settled** — a Meeting is publishable once it has a **real** (post-meeting) cached summary, or the clock has run out: 12 hours after the meeting sat with no recording (ADR `0027`), or seven days after it sat with a recording but no summary. A provisional summary does not settle it: it was written before the meeting from the agenda alone, and an entry settled on it would say what the agenda promised, not what happened (ADR `0026`). A meeting settles on its own, without waiting on a sibling that sat the same day. Nothing is published thin and filled in later: a reader who saw the thin version would never be shown the full one. A meeting that settles with no recording carries a not-recorded note in the feed (ADR `0026`), and settles onto its body's past tab on the site (ADR `0028`).
- **Recording state** — what a Meeting's detail page says where the video would be, when there is no video. Two distinct states, told apart by 12 hours after the meeting's start (ADR `0027`): **pending** (the City has not posted the recording yet; the site will transcribe and summarize it when it lands) and **not recorded** (12 hours have passed with no video, so the meeting was never recorded). The feed settles a no-video meeting on the same 12 hours, so the site and the feed flip together: a missing recording becomes a not-recorded note at the same moment. A cancelled meeting and one that has not happened yet say nothing.
