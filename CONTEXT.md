...
- **Index Card** — a Meeting's summary block on the index page, meant to get a reader into the right Meeting, not to list it. What it may show is bounded by a **Space Budget** of 15 units (roughly one mobile screen), spent on rows of different costs:
  ...
  When a card exceeds its budget it spends down in a fixed order (ADR `0020`): Speaker Rows collapse to the digest first, then Detailed Rows demote to Title-Only, then Title-Only Rows drop from the bottom as a last resort. Breadth over depth: a packed meeting names more of what council did, not fewer things in more words.
